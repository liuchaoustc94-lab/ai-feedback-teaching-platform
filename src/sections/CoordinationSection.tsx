      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="inline-flex items-center gap-2 bg-[#0a4fff]/10 text-[#0a4fff] text-xs font-mono-data px-3 py-1.5 rounded-full mb-6">
            <Move size={12} />
            {showMotor && showFeedback ? 'MODULE F4 · F5' : showMotor ? 'MODULE F4' : 'MODULE F5'}
          </span>
          <h2 className="font-serif-cn text-3xl md:text-4xl text-[#111] mb-4 leading-tight">
            {showMotor && showFeedback ? '动作协调、控制与反馈' : showMotor ? '动作协调与控制' : '反馈与运动学习'}
          </h2>
          <p className="text-[#555] leading-relaxed">
            {showMotor
              ? '打开摄像头，实时识别人体关节点，显示骨架轮廓与关键关节角度。追踪指定关节的运动轨迹，评估动作稳定性，实现练习前后的动作对比。'
              : '记录训练后的动作反馈与学习变化，按阶段关联指标，为再练习效果追踪提供依据。'}
          </p>
        </div>

        {/* Main Feature Display */}
        {showMotor && <div className="relative rounded-3xl overflow-hidden bg-[#111] mb-12">
        ...
        {showMotor && <div className="bg-[#134A34] rounded-2xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {showMotor && <div className="coord-card bg-white rounded-2xl p-6 border border-[#e5e5e5]">
          ...
          {showFeedback && <div className="coord-card bg-white rounded-2xl p-6 border border-[#e5e5e5]">
