...
const mocks = vi.hoisted(() => ({
  ...
  downloadFile: vi.fn(async () => new Blob(['report'], { type: 'text/plain' })),
  startCamera: vi.fn(),
  stopCamera: vi.fn(),
...
vi.mock('../lib/trainingArchive', () => ({
  savePoseReportToArchive: (...args: unknown[]) => {
    mocks.savePoseReportToArchive(...args)
...
describe('PoseAnalysisPage', () => {
  ...
  it('exports the generated report as a text file', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '停止并生成报告' }))
    await waitFor(() => expect(screen.getByRole('button', { name: '导出报告' })).toBeEnabled())
