...
import { ApiError, buildReportDownloadPath, downloadFile } from '../lib/api'
...
export default function PoseAnalysisPage() {
  ...
  useEffect(() => {
    ...
    setArchiveError('')
    Promise.resolve(savePoseReportToArchive(report, moduleCode, moduleTitle))
      .then((savedRecord) => {
        if (savedRecord) setSavedRecordId(savedRecord.id)
      })
      .catch((requestError: unknown) => {
        setArchiveError(requestError instanceof ApiError ? requestError.detail : '训练记录同步失败，请稍后重试')
      })
  ...
  const handleExportReport = async () => {
    ...
    try {
      const blob = await downloadFile(buildReportDownloadPath(savedRecordId))
      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `姿态分析报告_${new Date().toISOString().slice(0, 10)}.txt`
      link.click()
      URL.revokeObjectURL(url)
    } catch (requestError) {
      setArchiveError(requestError instanceof ApiError ? requestError.detail : '报告导出失败，请稍后重试')
    }
  }
              ...
              {phase === 'report' && (
                <>
                  ...
                  <button
                    ...
                    disabled={savingArchive || !savedRecordId}
                    className="inline-flex items-center gap-2 bg-[#111] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#333] transition-colors"
                  >
                    <Download size={15} />
                    {savingArchive ? '保存中…' : '导出报告'}
                  </button>
                </>
              )}
            </div>

            {archiveError && <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{archiveError}</div>}
